...
try:
    svc = joblib.load('./FaceDetection/model/svc.pkl')
...
mydict = get_name_list("./FaceDetection/data")
mydict = mydict
#           'NguyenVanAn', 'SangSang']
    ...
    if faces[1] is not None:
        for idx, face in enumerate(faces[1]):
            ...
            cv.putText(frame, result,
                       (coords[0], coords[1] - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5, (53, 204, 236), 2)
            cv.rectangle(frame, (coords[0], coords[1]), (coords[0] +
                         coords[2], coords[1]+coords[3]), (121, 50, 200), thickness)
            ...
            # cv.circle(frame, (coords[10], coords[11]),
            #           2, (255, 0, 255), thickness)
            # cv.circle(frame, (coords[12], coords[13]),
            #           2, (0, 255, 255), thickness)
    cv.putText(frame, 'FPS: {:.2f}'.format(tm.getFPS()), (1, 16),
               cv.FONT_HERSHEY_SIMPLEX, 0.5, (121, 129, 56), 2)  # 56, 129, 121  BGR
    return frame
